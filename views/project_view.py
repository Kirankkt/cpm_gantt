...
def show_project_view(project_id: int = 1) -> None:
    ...
    st.data_editor(
        ...
        num_rows="dynamic",
        key="task_grid",           # widget manages its own state
    )
    ...
    if st.button("💾 Save to DB & Re-calculate", type="primary"):
        ...
        st.rerun()                 # reload UI with new canonical data

    # -------- 4 · build CPM & charts from widget value ------------
    working_df = pd.DataFrame(st.session_state.get("task_grid", df_tasks))
